...
SerialPort::SerialPort(char *portName)
{
	...
	if (fd < 0)
	{
		...
		this->connected = true;
		this->setup(this->fd, B115200, 0);
		this->setBlocking(this->fd, 0);
	}
}
...
int SerialPort::readSerialPort(char *data)
{
}

bool SerialPort::writeSerialPort(std::string data)
{
	ROS_INFO("%i %s", data.size(), data.c_str());
...
int SerialPort::setup (int fd, int speed, int parity)
{
        struct termios tty;
